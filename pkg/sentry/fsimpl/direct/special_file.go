...
func newSpecialFileFD(h handle, mnt *vfs.Mount, d *dentry, flags uint32) (*specialFileFD, error) {
	...
	haveQueue := (ftype == linux.S_IFIFO || ftype == linux.S_IFSOCK)
	fd := &specialFileFD{
		handle:        h,
	...
	if haveQueue {
		if err := fdnotifier.AddFD(int32(h.fd), &fd.queue); err != nil {
			return nil, err
		}
	}
	if err := fd.vfsfd.Init(fd, flags, mnt, &d.vfsd, &vfs.FileDescriptionOptions{
		...
		if haveQueue {
			fdnotifier.RemoveFD(int32(h.fd))
		}
		return nil, err
	}
	d.fs.syncMu.Lock()
...
func (fd *specialFileFD) Release(ctx context.Context) {
	if fd.haveQueue {
		fdnotifier.RemoveFD(int32(fd.handle.fd))
	}
	fd.releaseMu.Lock()
	fd.handle.close()
	fd.releaseMu.Unlock()
...
func (fd *specialFileFD) Readiness(mask waiter.EventMask) waiter.EventMask {
	if fd.haveQueue {
		return fdnotifier.NonBlockingPoll(int32(fd.handle.fd), mask)
	}
...
func (fd *specialFileFD) EventRegister(e *waiter.Entry) error {
	if fd.haveQueue {
		...
		if err := fdnotifier.UpdateFD(int32(fd.handle.fd)); err != nil {
...
func (fd *specialFileFD) Allocate(ctx context.Context, mode, offset, length uint64) error {
	if fd.isRegularFile {
		...
		return d.doAllocate(ctx, offset, length, func() error {
			return unix.Fallocate(fd.handle.fd, uint32(mode), int64(offset), int64(length))
		})
	}
...
func (fd *specialFileFD) pwrite(ctx context.Context, src usermem.IOSequence, offset int64, opts vfs.WriteOptions) (written, finalOff int64, err error) {
	...
	if n > 0 && fd.vfsfd.StatusFlags()&(linux.O_DSYNC|linux.O_SYNC) != 0 {
		// Note that if syncing the host file fails, then we can't guarantee that
...
func (fd *specialFileFD) sync(ctx context.Context, forFilesystemSync bool) error {
	...
	if err != nil {
		...
		ctx.Debugf("direct.specialFileFD.sync: syncing non-writable or non-regular-file FD failed: %v", err)
	}
	return nil
}
...
func (fd *specialFileFD) requireHostFD() {
	if !fd.handle.isOpen() {
		...
		panic("direct.specialFileFD can no longer be memory-mapped without a host FD")
	}
}

// updateMetadata is called to sync d's metadata with the host.
func (fd *specialFileFD) updateMetadata() error {
	d := fd.dentry()
	d.metadataMu.Lock()
	defer d.metadataMu.Unlock()

	var stat unix.Stat_t
	if err := unix.Fstat(fd.handle.fd, &stat); err != nil {
		return err
	}
