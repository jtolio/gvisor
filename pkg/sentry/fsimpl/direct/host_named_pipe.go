...
func init() {
	...
	if err := unix.Pipe(pipeFDs[:]); err != nil {
		panic(fmt.Sprintf("failed to create pipe for direct.blockUntilNonblockingPipeHasWriter: %v", err))
	}
	tempPipeReadFD = pipeFDs[0]
	tempPipeWriteFD = pipeFDs[1]
}

func blockUntilNonblockingPipeHasWriter(ctx context.Context, fd int) error {
	for {
...
func nonblockingPipeHasWriter(fd int) (bool, error) {
	tempPipeMu.Lock()
	defer tempPipeMu.Unlock()
	// Copy 1 byte from fd into the temporary pipe.
	n, err := unix.Tee(fd, tempPipeWriteFD, 1, unix.SPLICE_F_NONBLOCK)
	...
	if n, err := unix.Read(tempPipeReadFD, tempPipeBuf[:]); err != nil || n != 1 {
		panic(fmt.Sprintf("failed to drain pipe for direct.blockUntilNonblockingPipeHasWriter: got (%d, %v), wanted (1, nil)", n, err))
	}
	return true, nil
}
