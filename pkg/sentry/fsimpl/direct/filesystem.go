...
func (fs *filesystem) Sync(ctx context.Context) error {
	...
	for _, d := range ds {
		if err := d.syncfsHostFile(ctx); err != nil {
			ctx.Infof("direct.filesystem.Sync: dentry.syncCachedFile failed: %v", err)
			if retErr == nil {
				retErr = err
			}
		}
	...
	for _, sffd := range sffds {
		if err := sffd.sync(ctx, true /* forFilesystemSync */); err != nil {
			ctx.Infof("direct.filesystem.Sync: specialFileFD.sync failed: %v", err)
			if retErr == nil {
				retErr = err
			}
		}
...
// MaxFilenameLen is the same as NAME_MAX.
const MaxFilenameLen = linux.NAME_MAX
...
func (fs *filesystem) stepLocked(ctx context.Context, rp *vfs.ResolvingPath, d *dentry, mayFollowSymlinks bool, ds **[]*dentry) (*dentry, error) {
	...
	if name == ".." {
		...
		// elsewhere in the host filesystem so that its parent has changed,
		...
		if d != d.parent && !d.cachedMetadataAuthoritative() {
			if err := d.parent.updateMetadata(); err != nil {
				return nil, err
			}
		}
		rp.Advance()
	...
	if child.isSymlink() && mayFollowSymlinks && rp.ShouldFollowSymlink() {
		target, err := child.readlink(rp.Mount())
		if err != nil {
			return nil, err
		}
...
func (fs *filesystem) getChildLocked(ctx context.Context, vfsObj *vfs.VirtualFilesystem, parent *dentry, name string, ds **[]*dentry) (*dentry, error) {
	...
	if child, ok := parent.children[name]; ok {
		...
		if child != nil {
			// Do not trust the child since !parent.cachedMetadataAuthoritative().
			// Revalidate it.
			if childValid, err := parent.revalidateChild(ctx, vfsObj, child, ds); err != nil {
				return nil, err
			} else if childValid {
				return child, nil
			}
			// In case the child was invalidated, re-create it from host. Continue.
		}
	}

	if parent.childrenSet != nil {
		// Is the child even there? Don't make the syscall if not.
		if _, ok := parent.childrenSet[name]; !ok {
			return nil, linuxerr.ENOENT
		}
	}

	childHandle, err := tryOpen(func(flags int) (int, error) {
		return unix.Openat(parent.controlHandle.fd, name, flags, 0)
	})
	if err != nil {
	...
	child, err := fs.newDentry(childHandle)
	if err != nil {
		childHandle.close()
		return nil, err
	}
...
// revalidateChild revalidates child by stat(2)-ing the host file again and
// checking that the file's inode number/device ID have not changed since
// we cached it in the dentry tree. It returns true if the child is valid.
//
// Preconditions: Same as getChildLocked and:
// * !parent.cachedMetadataAuthoritative()
func (d *dentry) revalidateChild(ctx context.Context, vfsObj *vfs.VirtualFilesystem, child *dentry, ds **[]*dentry) (bool, error) {
	...
	// host and release it after updating local metadata.
	child.metadataMu.Lock()
	defer child.metadataMu.Unlock()

	stat, err := statAt(d.controlHandle.fd, child.name)
	if err != nil && err != unix.ENOENT {
		return false, err
	}
	childPresent := err == nil // err is either nil or ENOENT here.

	if childPresent && child.inoKey == inoKeyFromHost(&stat) {
		// The file at this path hasn't changed. Just update cached metadata.
		child.updateMetadataFromStatLocked(&stat)
		return true, nil
	}
	if !childPresent && child.isSynthetic() {
		// We have a synthetic file, and no host file has arisen to
		// replace it.
		return true, nil
	}
	...
	*ds = appendDentry(*ds, child)
	if !childPresent {
		...
		// allowed.
		d.cacheNegativeLookupLocked(child.name)
...
func (fs *filesystem) walkParentDirLocked(ctx context.Context, rp *vfs.ResolvingPath, d *dentry, ds **[]*dentry) (*dentry, error) {
	if !d.cachedMetadataAuthoritative() {
		// Get updated metadata for d as required by fs.stepLocked().
...
// createInHostDir (if the parent directory is a real host directory) or
// createInSyntheticDir (if the parent directory is synthetic) to do so.
//
// Preconditions:
//   - !rp.Done().
//   - For the final path component in rp, !rp.ShouldFollowSymlink().
func (fs *filesystem) doCreateAt(ctx context.Context, rp *vfs.ResolvingPath, dir bool, createInHostDir func(parent *dentry, name string, ds **[]*dentry) error, createInSyntheticDir func(parent *dentry, name string) error) error {
	var ds *[]*dentry
	fs.renameMu.RLock()
	...
	if child, ok := parent.children[name]; ok && child != nil {
		...
		// Need to make sure child still exists before declaring EEXIST.
		if childValid, err := parent.revalidateChild(ctx, rp.VirtualFilesystem(), child, &ds); err == nil && childValid {
			return linuxerr.EEXIST
		}
	}
	if parent.childrenSet != nil {
	...
	checkExistence := func() error {
		if child, err := fs.getChildLocked(ctx, rp.VirtualFilesystem(), parent, name, &ds); err != nil && !linuxerr.Equals(linuxerr.ENOENT, err) {
			return err
		} else if child != nil {
			return linuxerr.EEXIST
		}
	...
	if err := createInHostDir(parent, name, &ds); err != nil {
		return err
	}
...
func (fs *filesystem) unlinkAt(ctx context.Context, rp *vfs.ResolvingPath, dir bool) error {
	...
	// deletion is allowed. If we need to revalidate, then too perform
	// fs.stepLocked() which performs the revalidation.
	var child *dentry
	if parent.mode.Load()&linux.ModeSticky == 0 && parent.cachedMetadataAuthoritative() {
		var ok bool
	...
	if parent.isSynthetic() {
		...
		if err := unix.Unlinkat(parent.controlHandle.fd, name, flags); err != nil {
			if child != nil {
				vfsObj.AbortDeleteDentry(&child.vfsd) // +checklocksforce: see above.
			}
			return err
...
func (fs *filesystem) LinkAt(ctx context.Context, rp *vfs.ResolvingPath, vd vfs.VirtualDentry) error {
	err := fs.doCreateAt(ctx, rp, false /* dir */, func(parent *dentry, childName string, ds **[]*dentry) error {
		...
		if err := unix.Linkat(d.controlHandle.fd, "", parent.controlHandle.fd, childName, unix.AT_EMPTY_PATH); err != nil {
			return err
		}
		cu := cleanup.Make(func() {
			// Best effort attempt to remove the hard link in case of failure.
			if err := unix.Unlinkat(parent.controlHandle.fd, childName, 0); err != nil {
				log.Warningf("error unlinking file %q after failure: %v", path.Join(genericDebugPathname(parent), childName), err)
			}
		})
		defer cu.Clean()

		linkHandle, err := tryOpen(func(flags int) (int, error) {
			return unix.Openat(parent.controlHandle.fd, childName, flags, 0)
		})
		if err != nil {
			return err
		}
		if err = parent.insertCreatedChildLocked(linkHandle, childName, nil, ds); err != nil {
			return err
		}
		cu.Release()
		return nil
...
func (fs *filesystem) MkdirAt(ctx context.Context, rp *vfs.ResolvingPath, opts vfs.MkdirOptions) error {
	...
	return fs.doCreateAt(ctx, rp, true /* dir */, func(parent *dentry, name string, ds **[]*dentry) error {
		...
		err := unix.Mkdirat(parent.controlHandle.fd, name, uint32(mode))
		if err == nil {
			cu := cleanup.Make(func() {
				// Best effort attempt to remove the dir in case of failure.
				if err := unix.Unlinkat(parent.controlHandle.fd, name, unix.AT_REMOVEDIR); err != nil {
					log.Warningf("error unlinking dir %q after failure: %v", path.Join(genericDebugPathname(parent), name), err)
				}
			})
			defer cu.Clean()

			childDirHandle, err := tryOpen(func(flags int) (int, error) {
				return unix.Openat(parent.controlHandle.fd, name, flags|unix.O_DIRECTORY, 0)
			})
			if err != nil {
				return err
			}
			if err := fchown(childDirHandle.fd, int(creds.EffectiveKUID), int(kgid)); err != nil {
				childDirHandle.close()
				return err
			}
			if err = parent.insertCreatedChildLocked(childDirHandle, name, nil, ds); err != nil {
				return err
			}
			cu.Release()
			if fs.opts.interop != InteropModeShared {
		...
		ctx.Infof("Failed to create host directory %q: %v; falling back to synthetic directory", name, err)
...
func (fs *filesystem) MknodAt(ctx context.Context, rp *vfs.ResolvingPath, opts vfs.MknodOptions) error {
	return fs.doCreateAt(ctx, rp, false /* dir */, func(parent *dentry, name string, ds **[]*dentry) error {
		...
		case linux.S_IFREG:
			if err := unix.Mknodat(parent.controlHandle.fd, name, uint32(opts.Mode), 0); err != nil {
				return err
			}
			cu := cleanup.Make(func() {
				// Best effort attempt to remove the file in case of failure.
				if err := unix.Unlinkat(parent.controlHandle.fd, name, 0); err != nil {
					log.Warningf("error unlinking file %q after failure: %v", path.Join(genericDebugPathname(parent), name), err)
				}
			})
			defer cu.Clean()
			...
			if err := fchown(childHandle.fd, int(creds.EffectiveKUID), int(creds.EffectiveKGID)); err != nil {
				childHandle.close()
				return err
			}
			if err := parent.insertCreatedChildLocked(childHandle, name, nil, ds); err != nil {
				return err
			}
			cu.Release()
			return nil
		case linux.S_IFSOCK:
			if _, ok := opts.Endpoint.(transport.HostBoundEndpoint); ok && fs.opts.hostUDS {
				return parent.bindAtLocked(ctx, name, creds, opts, ds)
			}

			// Check for file existence before creating synthetic socket.
			if _, err := fs.stepLocked(ctx, rp, parent, false, ds); err == nil {
			...
			*ds = appendDentry(*ds, parent)
			return nil
		case linux.S_IFIFO:
			// Check for file existence before creating synthetic pipe.
			if _, err := fs.stepLocked(ctx, rp, parent, false, ds); err == nil {
				// Step succeeded, another file exists.
				return linuxerr.EEXIST
			}
...
func (fs *filesystem) OpenAt(ctx context.Context, rp *vfs.ResolvingPath, opts vfs.OpenOptions) (*vfs.FileDescription, error) {
	// Reject O_TMPFILE for now, as the gofer client does.
	...
	if rp.Done() {
		...
		if !start.cachedMetadataAuthoritative() {
			...
			if err := start.updateMetadata(); err != nil {
				return nil, err
			}
		}
		start.IncRef()
...
func (d *dentry) open(ctx context.Context, rp *vfs.ResolvingPath, opts *vfs.OpenOptions) (*vfs.FileDescription, error) {
	...
	if trunc {
		...
		// open(2) will change the file size on host.
		d.metadataMu.Lock()
	...
	case linux.S_IFREG:
		if !d.fs.opts.regularFilesUseSpecialFileFD {
			if err := d.ensureSharedHandle(ats.MayRead(), ats.MayWrite(), trunc); err != nil {
				return nil, err
			}
	...
	case linux.S_IFDIR:
		...
		if !d.isSynthetic() {
			if err := d.ensureSharedHandle(ats&vfs.MayRead != 0, false /* write */, false /* trunc */); err != nil {
				return nil, err
			}
		}
		fd := &directoryFD{}
...
func (d *dentry) openSpecialFile(ctx context.Context, mnt *vfs.Mount, opts *vfs.OpenOptions) (*vfs.FileDescription, error) {
	...
	// We assume that d.controlHandle is opened with O_NONBLOCK for all named
	// pipes, see tryOpen().
	//
...
retry:
	h, err := d.openHandle(ats.MayRead(), ats.MayWrite(), opts.Flags&linux.O_TRUNC != 0)
	if err != nil {
	...
	if isBlockingOpenOfNamedPipe && ats == vfs.MayRead {
...
func (d *dentry) createAndOpenChildLocked(ctx context.Context, rp *vfs.ResolvingPath, opts *vfs.OpenOptions, ds **[]*dentry) (*vfs.FileDescription, error) {
	...
	const createFlags = unix.O_CREAT | unix.O_EXCL | unix.O_RDONLY | unix.O_NONBLOCK | hostOpenFlags
	childHostFD, err := unix.Openat(d.controlHandle.fd, name, createFlags, uint32(opts.Mode&^linux.FileTypeMask))
	if err != nil {
		return nil, err
	}
	childHandle := handle{fd: childHostFD}

	cu := cleanup.Make(func() {
		// Best effort attempt to remove the file in case of failure.
		if err := unix.Unlinkat(d.controlHandle.fd, name, 0); err != nil {
			log.Warningf("error unlinking file %q after failure: %v", path.Join(genericDebugPathname(d), name), err)
		}
		childHandle.close()
	})
	defer cu.Clean()

	if err := fchown(childHostFD, int(creds.EffectiveKUID), int(kgid)); err != nil {
		return nil, err
	}
	...
	// TODO(gvisor.dev/issue/3626): Handle other flags.
	ats := vfs.AccessTypesForOpenFlags(opts)
	h, err := child.openHandle(ats.MayRead(), ats.MayWrite(), false /* trunc */)
	if err != nil {
		return nil, err
	}
	cu.Add(func() {
	...
	// Incorporate the FD that was opened into the dentry.
	useRegularFileFD := child.fileType() == linux.S_IFREG && !d.fs.opts.regularFilesUseSpecialFileFD
	if useRegularFileFD {
		child.handleMu.Lock()
		if vfs.MayReadFileWithOpenFlags(opts.Flags) {
			child.readHandle = h
			child.mmapHandle = child.readHandle
		}
		if vfs.MayWriteFileWithOpenFlags(opts.Flags) {
			child.writeHandle = h
			child.mmapHandle = child.writeHandle
		}
...
func (fs *filesystem) RenameAt(ctx context.Context, rp *vfs.ResolvingPath, oldParentVD vfs.VirtualDentry, oldName string, opts vfs.RenameOptions) error {
	...
	if fs.opts.interop == InteropModeShared && opts.Flags&linux.RENAME_NOREPLACE != 0 {
		// Reject this for now, as gofer clients do.
		return linuxerr.EINVAL
	}
	...
	if !oldParent.cachedMetadataAuthoritative() {
		if err := oldParent.updateMetadata(); err != nil {
			return err
		}
	}
	creds := rp.Credentials()
	...
	renamed, err := fs.getChildLocked(ctx, rp.VirtualFilesystem(), oldParent, oldName, &ds)
	if err != nil {
		return err
	}
	...
	if !renamed.isSynthetic() {
		if err := renameat(oldParent.controlHandle.fd, oldName, newParent.controlHandle.fd, newName); err != nil {
		...
		if err := unix.Unlinkat(newParent.controlHandle.fd, newName, flags); err != nil {
			vfsObj.AbortRenameDentry(&renamed.vfsd, replacedVFSD)
			return err
		}
	}
...
func (fs *filesystem) StatFSAt(ctx context.Context, rp *vfs.ResolvingPath) (linux.Statfs, error) {
	...
	if err := unix.Fstatfs(d.controlHandle.fd, &statFS); err != nil {
		return linux.Statfs{}, err
	}
	if statFS.Namelen == 0 || statFS.Namelen > int64(MaxFilenameLen) {
		statFS.Namelen = int64(MaxFilenameLen)
	}
	return linux.Statfs{
		// This is primarily for distinguishing this file system in
...
func (fs *filesystem) SymlinkAt(ctx context.Context, rp *vfs.ResolvingPath, target string) error {
	return fs.doCreateAt(ctx, rp, false /* dir */, func(parent *dentry, name string, ds **[]*dentry) error {
		creds := rp.Credentials()
		if err := unix.Symlinkat(target, parent.controlHandle.fd, name); err != nil {
			return err
		}
		cu := cleanup.Make(func() {
		...
		symlinkFD, err := unix.Openat(parent.controlHandle.fd, name, unix.O_PATH|hostOpenFlags, 0)
		if err != nil {
			return err
		}
		symlinkHandle := handle{fd: symlinkFD}
		if err := fchown(symlinkFD, int(creds.EffectiveKUID), int(creds.EffectiveKGID)); err != nil {
			symlinkHandle.close()
			return err
		}
		if err := parent.insertCreatedChildLocked(symlinkHandle, name, func(child *dentry) {
			child.target = target
...
func (fs *filesystem) MountOptions() string {
	optsKV := []mopt{
		{moptSockFD, fs.opts.goferSock.FD()},
	}
	...
	case InteropModeShared:
		optsKV = append(optsKV, mopt{moptCache, cacheRevalidating})
	}
